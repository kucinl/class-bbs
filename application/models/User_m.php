...
class User_m extends CI_Model
{
	...
	/*login in*/
    function login($data){
	    $user = $this->get_user_by_uid($data['uid']);
	    if($user){
			$password = password_dohash($data['password'],$user['salt']);
			if($user['password']==$password){
				$this->session->set_userdata(array ('uid' => $user['uid'], 'username' => $user['username'], 'group_type' => $user['group_type'], 'gid' => $user['gid'], 'avatar' => $user['avatar'], 'group_name' => $user['group_name'], 'is_active' => $user['is_active'], 'favorites' => $user['favorites'], 'follows' => $user['follows'], 'notices' => $user['notices'], 'messages_unread' => $user['messages_unread'], 'credit' => $user['credit'], 'lastpost' => $user['lastpost'],'lastlogin' => $user['lastlogin']));
				return TRUE;
			} else {
				return FALSE;
			}
	...
	{
		return ($this->is_login() && $uid==$this->session->uid) ? TRUE : FALSE;
	}
	//判断是否是管理员
	public function is_admin()
	{
		$group_type=$this->session->group_type;
		//管理员号为0
	...
	{
		$uid = $this->session->uid;
		$query = $this->db->select('uid')->get_where('user_nodes', array('node_id'=>$node_id,'uid'=>$uid,'is_master'=>1));
		if($query->num_rows() !== 0){
			return true;
		}
        return false;
    ...
    //判断是否是节点成员
    public function is_member($node_id)
	{
        $uid = $this->session->uid;
		$query = $this->db->select('uid')->get_where('user_nodes', array('node_id'=>$node_id,'uid'=>$uid));
		if($query->num_rows() !== 0){
			return true;
		}
        return false;
    ...
    //判断是否允许修改此主题
    public function permit_edit($topic_id)
	{
